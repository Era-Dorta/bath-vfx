...
Normal maps synthesis also required some pre and post-processing steps for the pipeline to work.
The ground truth normal maps that we use as $\left\lbrace A, A' \right\rbrace$ were taken from Graham et al.\cite{Graham2013}.
...
To generate the results with the image super-resolution technique from Jianchao et al.~\cite{Jianchao2010}, we used the image dictionaries provided by the authors.
...
Dividing the face into segments exploits more efficiently the nowadays common multi-core CPU architectures, as we can ran the algorithm for each patch independently.
