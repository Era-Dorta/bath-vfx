...
A k-d tree for an Approximate Neighbour Search is built using a feature vector from a neighbour pixel $p$ in $A$ and $A'$.
...
A first approach we tried to use the Image Analogies filter to create increase quality textures.
...
The texture segmentation is shown in Figure and results are shown in Figure .  
...
% Their original code, the matlab one, the c++ one, and the cuda one
% Replicate Graham2013, bump map synthesis
% Bump maps from texture and then bump map increase
% Texture resolution increase
% Normal generation
