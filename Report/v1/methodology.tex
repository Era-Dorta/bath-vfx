...
where $\vect{v} = (v_x, v_y)$ is the optical flow at pixel $(x, y)$ from the image frame $I$ at time $t$ to time $t+1$. The algorithm generates an image pyramid where the image resolution is halved on each pyramid level.
