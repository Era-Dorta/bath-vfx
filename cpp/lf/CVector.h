...
public:
	...
	void copy(T * data) {
		memcpy(_data, data, _length * sizeof(T));
	}
...
public:
	...
	Point2(void) :
			location(-1, -1), imageID(-1) {
	}
	Point2(const Point2 &p) :
			location(p.location), imageID(p.imageID) {
	}
	Point2(int ix, int iy) :
			location(ix, iy), imageID(-1) {
	}
	Point2(int ix, int iy, int imageID) :
			location(ix, iy), imageID(imageID) {
	}
	bool operator==(const Point2 & p) {
		return imageID == p.imageID && location == p.location;
	}
	bool operator!=(const Point2 & p) {
		return !(*this == p);
	}
	bool operator<(const Point2 & p) const {
		return imageID < p.imageID
				|| (imageID == p.imageID && location < p.location);
	}
	bool operator>(const Point2 & p) const {
		return imageID > p.imageID
				|| (imageID == p.imageID && location > p.location);
...
struct Neigh: public CVector<double> {
	...
	Neigh(int length, Point2 p) :
			CVector<double>(length), loc(p), constSrc(false), srcPixelDim(0) {
	}
	Neigh(const CVector<double> & v) :
			CVector<double>(v), loc(-1, -1, -1), constSrc(false), srcPixelDim(0) {
	}
	Neigh(const Neigh & n) :
			CVector<double>(n), loc(n.loc), constSrc(n.constSrc), srcPixelDim(
					n.srcPixelDim) {
		if (n.constSrc)
