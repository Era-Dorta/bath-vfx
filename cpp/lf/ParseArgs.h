...
typedef void DoubleCallbackType(const char *name, double *var,
		ParseArgs *parser);
...
typedef void StringCallbackType(const char *name, char *var, int maxLen,
		ParseArgs *parser);
...
public:
	...
	void AddEnumParam(const char *name, EnumValueInfo *info, int numVals,
			int *var, EnumCallbackType *callback = NULL); // really an int, with names for values
	void AddStringParam(const char *name, char *var, int maxLength,
			StringCallbackType *callback = NULL);
	enum {
		MAX_NAME_LENGTH = 100
	};
