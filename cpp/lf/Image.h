...
enum SynthProcedureType {
	FILTER_SYNTH = 0, COPY_SYNTH = 1, FILTER_NO_SOURCE_SYNTH = 2, ZERO_SYNTH = 3
};
enum PyramidType {
	GAUSSIAN_PYRAMID = 0,
	LAPLACIAN_PYRAMID = 1,
...
enum FeatureType {
	RAW_RGB_FEATURE = 0, DIFFERENCE_FEATURE = 1, LUV_FEATURE = 2
};
...
//  Image(MATRIX &m);
	...
	void clear() {
		memset(_pixels, 0, _width * _height * _dim * sizeof(T));
	}
	...
	void applyGainBias(const CMatrix<double>& gain,
			const CVector<double> & bias);
