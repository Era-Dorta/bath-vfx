...
public:
	Sampler(float epsilon = .1) :
			_epsilon(epsilon), _threshold(-1) {
	}
	...
	pair<float, T> UniformSample() {
		...
		typename multimap<float, T>::iterator it = _data.upper_bound(
				_threshold);
		if (it != _data.end())
	...
	void print() const {
		printf("samples: ");
		for (typename multimap<float, T>::const_iterator it2 = _data.begin();
				it2 != _data.end(); ++it2)
			printf("%f (%d %d)", (*it2).first, (*it2).second.x(),
					(*it2).second.y());
		printf("; epsilon = %f\n", _epsilon);
