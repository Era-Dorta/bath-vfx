...
public:
	CSearchEnvironment(Sampler<Point2> *sampler) :
			m_sampler(sampler) {
	}
	virtual ~CSearchEnvironment(void) {
	}

	void SetSampler(Sampler<Point2> * sampler) {
		m_sampler = sampler;
	}
...
public:
	...
	// setting things
	void SetANNEpsilon(float epsilon) {
		m_annEpsilon = epsilon;
	}
...
public:
	...
	// setting things
	void SetNumComparisons(int n) {
		m_numComparisons = n;
	}
	void SetBacktracks(int n) {
		m_TSVQbacktracks = n;
	}
