...
TSVQR::TSVQR(const vector<Neigh*> & inputData,
		const vector<CVector<double>*> & targetData, int maxDepth,
		Neigh *cent1) {
	...
	do {
		...
		if (npts1 == 0) {
			assert(n == 1);
			m_center1 = *inputData[0];
			npts1 = 1;
			m_center2 -= *inputData[0];
			npts2--;
		} else if (npts2 == 0) {
			assert(n == 1);
			m_center2 = *inputData[0];
...
CVector<double> TSVQR::apply(const CVector<double> & pt,
		unsigned int & numComparisons, int numBacktracks) const {
...
Neigh TSVQR::findNearest(const CVector<double> & pt,
		unsigned int & numComparisons, int numBacktracks) const {
	const TSVQR * nearestLeaf = findNearestHelper(pt, numComparisons,
			numBacktracks);
	//  return (*nearestLeaf->m_A)*pt + *nearestLeaf->m_b;
...
const TSVQR * TSVQR::findNearestHelper(const CVector<double> & pt,
		unsigned int &numComparisons, int &numBacktracks) const {
	if (m_leafNode) {
		numBacktracks--;
		return this;
	...
	const TSVQR * result = nearChild->findNearestHelper(pt, numComparisons,
			numBacktracks);
	if (numBacktracks > 0) {
		const TSVQR * newResult = farChild->findNearestHelper(pt,
				numComparisons, numBacktracks);
