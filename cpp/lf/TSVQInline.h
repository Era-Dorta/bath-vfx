...
TSVQ<PointN>::TSVQ(const vector<PointN*> & points, float maxError, int maxDepth,
		PointN *cent1) {
...
template<class PointN>
const PointN &TSVQ<PointN>::findNearestHelper(const PointN & pt,
		unsigned int &numComparisons, int &numBacktracks) {
	if (m_leafNode) {
		numBacktracks--;
		return m_point;
	...
	const PointN &result = nearChild->findNearestHelper(pt, numComparisons,
			numBacktracks);
	if (numBacktracks > 0) {
		const PointN &newResult = farChild->findNearestHelper(pt,
...
template<class PointN>
const void TSVQ<PointN>::sample(const PointN &pt, Sampler<Point2> & samp,
		unsigned int & numComparisons, int numBacktracks) {
	sampleHelper(pt, samp, numComparisons, numBacktracks);
}

template<class PointN>
const void TSVQ<PointN>::sampleHelper(const PointN &pt, Sampler<Point2> & samp,
		unsigned int & numComparisons, int & numBacktracks) {
	if (m_leafNode) {
		numBacktracks--;
