...
void computeOutput(int redisplay = 0) {
	...
	fl.maxTSVQdepth() = (
			searchType == TLearner::HEURISTIC_SEARCH ?
	...
	fl.annEpsilon() = (
			searchType == TLearner::HEURISTIC_ANN_SEARCH ?
...
bool winxyToImagexy(int wx, int wy, int &imx, int &imy) {
	...
	if (result) {
		...
		if (sourceX >= 0) {
			fl.filteredExamplePyramid()[level].imagexyToWinxy(sourceX, sourceY,
					imx, imy);
		} else {
			imx = -1;
			imy = -1;
...
void mousePassiveMotion(int wx, int wy) {
	...
	bool result = winxyToImagexy(motionWX, motionWY, motionSourceWX,
			motionSourceWY);
...
void display() {
	...
	if (showSources) {
		fl.sourcesPyramid().drawSourcesColumn(x1, 0,
				fl.filteredExamplePyramid()[0].width(),
...
void saveSummary(int = 0) {
	...
	if (useSourceImages) {
		...
		if (sourceExampleImages[pairIndex]
				!= inputSourceExampleImages[pairIndex] && !convertToYIQ)
			exampleRowWidth +=
					(inputSourceExampleImages[pairIndex]->width() + 5);
		if (filteredExampleImages[pairIndex]
				!= inputFilteredExampleImages[pairIndex] && !convertToYIQ)
			exampleRowWidth += (inputFilteredExampleImages[pairIndex]->width()
					+ 5);
		...
		int height = filteredExampleImages[pairIndex]->height()
				+ filteredImage->height() + 5;
		...
		if (inputSourceExampleImages[pairIndex]
				!= sourceExampleImages[pairIndex] && !convertToYIQ) {
		...
		if (inputFilteredExampleImages[pairIndex]
				!= filteredExampleImages[pairIndex] && !convertToYIQ) {
		...
		int width = filteredImage->width()
				+ filteredExampleImages[pairIndex]->width() + 5;
		int height = max(filteredImage->height(),
				filteredExampleImages[pairIndex]->height());

		if (inputFilteredExampleImages[pairIndex]
				!= filteredExampleImages[pairIndex])
...
void writeHTMLFile(void) {
	...
	summaryHTMLFile << "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 3.2//EN\">"
			<< endl;
	...
	summaryHTMLFile
			<< "bgcolor=\"#ffffe0\" text=\"#100000\" vlink=\"#101030\" link=\"#002040\" alink=\"#008000\">"
			<< endl;
...
void reshape(int w, int h) {
	...
	glLoadIdentity();
	glOrtho(0.0f, (GLfloat) windowWidth, 0.0f, (GLfloat) windowHeight, -1.0f,
			1.0f);
...
// transform to target2 if target2 is non-NULL.
	...
	if (dim == 1) {
		...
		CVector<double> eigenvalsSE = sourceExampleCovar.symmetricEigenvectors(
				eigenvectorsSE, dim, 1);
		CMatrix<double> eigenvectorsS(dim, dim);
		CVector<double> eigenvalsS = sourceCovar.symmetricEigenvectors(
				eigenvectorsS, dim, 1);
		...
		CMatrix<double> gain = eigenvectorsS * sigmaS
				* eigenvectorsS.transpose() * eigenvectorsSE * sigmaSE
				* eigenvectorsSE.transpose();
...
void SetupImages(int) {
	...
	for (index = 0; index < inputSourceExampleImages.size(); index++)
		inputSourceExampleImages[index]->ConvertToColorspace(
	...
	for (index = 0; index < inputFilteredExampleImages.size(); index++)
		inputFilteredExampleImages[index]->ConvertToColorspace(
	...
	for (index = 0; index < filteredExampleImages.size(); index++) {
		if (filteredExampleImages[index] != NULL) {
			if (index >= inputFilteredExampleImages.size()
					|| filteredExampleImages[index]
							!= inputFilteredExampleImages[index])
	...
	for (index = 0; index < inputFilteredExampleImages.size(); index++) {
		if (convertToYIQ) {
			filteredExampleImages.push_back(
					inputFilteredExampleImages[index]->luminance());
		} else if (filteredPyramidType != GAUSSIAN_PYRAMID
				|| filteredFeatureType != RAW_RGB_FEATURE || matchGrayHistogram
				|| histogramEq || matchMeanVariance) {
			// create a copy of the existing image
			filteredExampleImages.push_back(
					new ColorImage(*inputFilteredExampleImages[index]));
		} else
	...
	for (index = 0; index < sourceExampleImages.size(); index++) {
		if (index >= inputSourceExampleImages.size()
				|| sourceExampleImages[index]
						!= inputSourceExampleImages[index])
	...
	if (convertToYIQ) {
		for (index = 0; index < inputSourceExampleImages.size(); index++)
			sourceExampleImages.push_back(
					inputSourceExampleImages[index]->luminance());
		...
		for (index = 0; index < inputSourceExampleImages.size(); index++)
			sourceExampleImages.push_back(
					new ColorImage(*inputSourceExampleImages[index]));
	...
	if ((matchGrayHistogram || histogramEq || matchMeanVariance)) {
		if (sourceExampleImages.size() != 1) {
			printf(
					"Warning: can't do histogram matching on more than one example pair\n");
		...
		for (x = 0; x < sourceExampleImages[0]->width(); x++)
			for (y = 0; y < sourceExampleImages[0]->height(); y++) {
				sourceExampleImages[0]->Pixel(x, y, 0) =
						seHist.applyHistMatchFunc(
								sourceExampleImages[0]->Pixel(x, y, 0), sHist);
			}
	}
	...
	if (histogramEq) {
		if (sourceExampleImages.size() != 1)
			printf(
					"Warning: histogram eq only works for a single example pair!\n");

		Histogram seHist(*sourceExampleImages[0], 1.0 / 256);

		for (x = 0; x < sourceExampleImages[0]->width(); x++)
			for (y = 0; y < sourceExampleImages[0]->height(); y++)
				sourceExampleImages[0]->Pixel(x, y, 0) = seHist.applyHistEqFunc(
						sourceExampleImages[0]->Pixel(x, y, 0));
		...
		for (x = 0; x < sourceImage->width(); x++)
			for (y = 0; y < sourceImage->height(); y++)
				sourceImage->Pixel(x, y, 0) = sHist.applyHistEqFunc(
	...
	if (matchMeanVariance) {
		for (int i = 0; i < sourceExampleImages.size(); i++)
			matchMV(sourceImage, sourceExampleImages[i],
					filteredExampleImages[i]);
	}
		...
		if (filteredFeatureType == DIFFERENCE_FEATURE)
			for (x = 0; x < filteredExampleImages[index]->width(); x++)
				for (y = 0; y < filteredExampleImages[index]->height(); y++)
					for (int d = 0; d < filteredExampleImages[index]->dim();
							d++)
	...
	if (useSourceImages) {
		 ...
		 */{
			fl.SetImages(sourceExampleImages, filteredExampleImages,
					sourceImage, pyramidHeight,
					useFilterModeMask ? filterModeMask : NULL,
					useTargetModeMask ? targetModeMask : NULL);
		}
	} else
	...
	if (oldSourcesImage != NULL) {
		if (oldSourcesImage->width() != fl.sourcesPyramid().FineImage().width()
...
//  int outputWidth;
	...
	if (showSource) {
		...
		windowHeight = max(windowHeight, fl.sourcePyramid().displayHeight());
	}

	if (showFiltered) {
		windowWidth += fl.filteredPyramid().displayWidth();
		windowHeight = max(windowHeight, fl.filteredPyramid().displayHeight());
	}
	...
	if (showSources) {
		...
		windowHeight = max(windowHeight, fl.sourcesPyramid().displayHeight());
	}

	if (showModeMask) {
		windowWidth += fl.modeMaskPyramid().displayWidth();
		windowHeight = max(windowHeight, fl.modeMaskPyramid().displayHeight());
	}
...
void checkGrayscale() {
	...
	for (index = 0; index < inputFilteredExampleImages.size(); index++) {
		if (!(inputFilteredExampleImages[index]->dim() > 1
				&& inputFilteredExampleImages[index]->dim()
						== inputSourceExampleImages[index]->dim()
				&& inputFilteredExampleImages[index]->dim()
			...
			for (y = 0; y < inputFilteredExampleImages[index]->height(); y++)
				for (d = 1; d < inputFilteredExampleImages[index]->dim(); d++)
					if (inputFilteredExampleImages[index]->Pixel(x, y, 0)
							!= inputFilteredExampleImages[index]->Pixel(x, y,
									d)) {
						grayscaleMode = false;
						printf("No.\n");
						return;
			...
			for (y = 0; y < inputSourceExampleImages[index]->height(); y++)
				for (d = 1; d < inputSourceExampleImages[index]->dim(); d++)
					if (inputSourceExampleImages[index]->Pixel(x, y, 0)
							!= inputSourceExampleImages[index]->Pixel(x, y,
									d)) {
						grayscaleMode = false;
						printf("No.\n");
						return;
	...
	for (index = 0; index < inputFilteredExampleImages.size(); index++) {
		ColorImage * filteredEx = new ColorImage(
				inputFilteredExampleImages[index]->width(),
				inputFilteredExampleImages[index]->height(), 1);
		ColorImage * sourceEx = new ColorImage(
				inputSourceExampleImages[index]->width(),
				inputFilteredExampleImages[index]->height(), 1);
		...
		for (x = 0; x < inputFilteredExampleImages[index]->width(); x++)
			for (y = 0; y < inputFilteredExampleImages[index]->height(); y++)
				filteredEx->Pixel(x, y, 0) =
						inputFilteredExampleImages[index]->Pixel(x, y, 0);

		for (x = 0; x < inputSourceExampleImages[index]->width(); x++)
			for (y = 0; y < inputSourceExampleImages[index]->height(); y++)
				sourceEx->Pixel(x, y, 0) =
						inputSourceExampleImages[index]->Pixel(x, y, 0);
...
void reconstruct(int) {
	...
	if (convertToYIQ) {
		...
		outputPyramid = new Pyramid<ColorImage>(outputPyramid->NLevels(),
				outputFilteredImage->width(), outputFilteredImage->height(), 3);
		outputFilteredImage = &outputPyramid->FineImage();
		outputPyramid->clear();
		...
		for (int x = 0; x < outputFilteredImage->width(); x++)
			for (int y = 0; y < outputFilteredImage->height(); y++) {
				...
				outputFilteredImage->Pixel(x, y, 0) = r2
						* outputFilteredImage->maxVal();
				outputFilteredImage->Pixel(x, y, 1) = g2
						* outputFilteredImage->maxVal();
				outputFilteredImage->Pixel(x, y, 2) = b2
						* outputFilteredImage->maxVal();
			}
...
void createUI() {
	...
	if (filterModeMask != NULL || targetModeMask != NULL) {
		...
		if (filterModeMask != NULL)
			glui->add_checkbox_to_panel(mmp, "Use Filter MM",
					&useFilterModeMask, 1, SetupImages);

		if (targetModeMask != NULL)
			glui->add_checkbox_to_panel(mmp, "Use Target MM",
					&useTargetModeMask, 1, SetupImages);
	...
	glui->add_spinner("Pyramid height", GLUI_SPINNER_INT, &pyramidHeight, 1,
			SetupImages)->set_int_limits(1, 10);
	...
	GLUI_RadioGroup * searchrg = glui->add_radiogroup_to_panel(span,
			&searchType);
	...
	glui->add_spinner_to_panel(span, "Passes", GLUI_SPINNER_INT, &numPasses)->set_int_limits(
			1, 3);
	...
	glui->add_checkbox_to_panel(span, "Cheesy boundaries", &cheesyBoundaries);
	glui->add_checkbox_to_panel(span, "Ashikhmin last level",
			&ashikhminLastLevel);
	...
	glui->add_spinner_to_panel(tsr, "Max TSVQ depth", GLUI_SPINNER_INT,
			&maxTSVQdepth);
	glui->add_spinner_to_panel(tsr, "Heur Max TSVQ depth", GLUI_SPINNER_INT,
			&heurMaxTSVQdepth);
	glui->add_spinner_to_panel(tsr, "Max TSVQ error", GLUI_SPINNER_FLOAT,
			&maxTSVQerror);
	glui->add_spinner_to_panel(tsr, "TSVQ backtrack", GLUI_SPINNER_INT,
			&TSVQbacktracks);
	...
	glui->add_spinner_to_panel(sampst, "Epsilon", GLUI_SPINNER_FLOAT,
			&samplerEpsilon)->set_float_limits(0, 1e10);
	...
	glui->add_checkbox_to_panel(hpn, "Match A/A' to B", &matchMeanVariance, 1,
			SetupImages);
	glui->add_checkbox_to_panel(hpn, "Match B to A", &matchBtoA, 1,
			SetupImages);
	/*
	...
	GLUI_RadioGroup * sptrg = glui->add_radiogroup_to_panel(spt,
			&sourcePyramidType, 1, SetupImages);
	...
	GLUI_RadioGroup * fptrg = glui->add_radiogroup_to_panel(fpt,
			&filteredPyramidType, 1, SetupImages);
	...
	GLUI_Checkbox * yiqcb = glui->add_checkbox_to_panel(ff, "YIQ",
			&convertToYIQ, 1, SetupImages);
	GLUI_RadioGroup * ffrg = glui->add_radiogroup_to_panel(ff,
			&filteredFeatureType, 1, SetupImages);
	glui->add_radiobutton_to_group(ffrg, "Raw");
	glui->add_radiobutton_to_group(ffrg, "Difference Image");

	GLUI_Panel * bspt = glui->add_panel_to_panel(fts,
			"Filtered Base Procedure");
	GLUI_RadioGroup * bsptrg = glui->add_radiogroup_to_panel(bspt,
			&filteredBaseProcedure, 1, SetupImages);
	...
	glui->add_checkbox_to_panel(dis, "Filtered example", &showFilteredExample,
			0, adjustWindowSize);
	glui->add_checkbox_to_panel(dis, "Example mode mask", &showExampleModeMask,
			0, adjustWindowSize);
...
void ArgFileCallback(const char *name, char *var, int maxLen,
		ParseArgs *parser) {
 ...
 */
void ExampleCallback(const char *name, char *var, int maxLen,
		ParseArgs *parser) {
...
void SetupArgParser(ParseArgs &parser) {
	...
	parser.AddStringParam("argFile:", argFilename, MAX_FILENAME_SIZE,
			ArgFileCallback);
	...
	EnumValueInfo methodValues[] = { { "Image", 0 }, { "Vector", 1 }, { "TSVQ",
			2 }, { "HeurTSVQ", 3 }, { "MLP", 4 }, { "TSVQR", 5 }, { "ANN", 6 },
			{ "HeurANN", 7 }, { "Ash", 8 } };
	parser.AddEnumParam("searchMethod:", methodValues, 9, &searchType);

	EnumValueInfo outputStyleValues[] = { { "Summary", SUMMARY }, { "Table",
			TABLE } };
	...
	EnumValueInfo pyramidTypeValues[] = { { "Gaussian", 0 }, { "Laplacian", 1 },
			{ "Steerable", 2 } };
	parser.AddEnumParam("pyramidType:", pyramidTypeValues, 3,
			&sourcePyramidType);
	parser.AddEnumParam("filteredPyramidType:", pyramidTypeValues, 2,
			&filteredPyramidType);

	parser.AddBoolParam("useYIQ:", &convertToYIQ);

	EnumValueInfo featureTypeValues[] = { { "Raw", 0 }, { "Difference", 1 } };
	parser.AddEnumParam("filteredFeatureType:", featureTypeValues, 2,
			&filteredFeatureType);

	// TODO: filtered base procedure enum
	EnumValueInfo filterProcTypeValues[] =
			{ { "Synthesize", 0 }, { "Copy", 1 } };
	parser.AddEnumParam("filterProcedure:", filterProcTypeValues, 2,
			&filteredBaseProcedure);

	EnumValueInfo colorspaceTypeValues[] = { { "RGB", RGB_SPACE }, { "XYZ",
			XYZ_SPACE }, { "Lab", LAB_SPACE }, { "Luv", LUV_SPACE } };
	parser.AddEnumParam("filterColorspace:", colorspaceTypeValues, 4,
			&filterColorspace);
	parser.AddEnumParam("sourceColorspace:", colorspaceTypeValues, 4,
			&sourceColorspace);
...
int main(int argc, char* argv[]) {
	...
	inputSourceExampleNames.erase(inputSourceExampleNames.begin(),
			inputSourceExampleNames.end());
	inputFilteredExampleNames.erase(inputFilteredExampleNames.begin(),
			inputFilteredExampleNames.end());
	...
	if (useSourceImages) {
		assert(
				inputSourceExampleNames.size()
						== inputFilteredExampleNames.size());

		printf("example pairs:\n");
		for (int index = 0; index < inputSourceExampleNames.size(); index++) {
			printf("src %d: %s\n", index,
					inputSourceExampleNames[index].c_str());
			printf("filt %d: %s\n", index,
					inputFilteredExampleNames[index].c_str());
		}
	} else {
		printf("example src:\n");
		for (int index = 0; index < inputSourceExampleNames.size(); index++)
	...
	int index;
	inputFilteredExampleImages.erase(inputFilteredExampleImages.begin(),
			inputFilteredExampleImages.end());
	...
	inputSourceExampleImages.erase(inputSourceExampleImages.begin(),
			inputSourceExampleImages.end());
	...
	for (index = 0; index < inputSourceExampleImages.size(); index++) {
		if (inputSourceExampleImages[index]->width()
				!= inputFilteredExampleImages[index]->width()
				|| inputSourceExampleImages[index]->height()
...
void saveTable(void) {
	...
	if (convertToYIQ) {
		...
		if (outputFilteredImage != NULL) {
			sprintf(buffer, "%s/%s", outputDirectoryName,
					outputFilteredYIQName);
	...
	outputHTMLFile << "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 3.2//EN\">"
			<< endl;
	...
	outputHTMLFile << "    <TD><IMG src=\"" << outputSourceExampleName
			<< "\"> </TD>" << endl;
	outputHTMLFile << "    <TD><IMG src=\"" << outputFilteredExampleName
			<< "\"> </TD>" << endl;
	outputHTMLFile << "    <TD><IMG src=\"" << outputFilteredExampleName
			<< "-gray\"> </TD>" << endl;
	outputHTMLFile << "  </TR>" << endl;
	...
	outputHTMLFile << "  <TR>" << endl;
	outputHTMLFile << "    <TD><IMG src=\"" << outputSourceName << "\"> </TD>"
			<< endl;
	outputHTMLFile << "    <TD><IMG src=\"" << outputSourceName
			<< "-filtered.png\"> </TD>" << endl;
	outputHTMLFile << "    <TD><IMG src=\"" << outputSourceName
			<< "-filtered-gray.png\"> </TD>" << endl;

	outputHTMLFile << "  <TR>" << endl;
	outputHTMLFile << "    <TD><IMG src=\"" << outputFilteredColorName
			<< "\"> </TD>" << endl;
	outputHTMLFile << "    <TD><IMG src=\"" << outputFilteredGrayName
			<< "\"> </TD>" << endl;
	outputHTMLFile << "    <TD><IMG src=\"" << outputFilteredYIQName
			<< "\"> </TD>" << endl;
