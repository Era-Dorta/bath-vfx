...
inline int imax(const int i1, const int i2) {
	return i1 > i2 ? i1 : i2;
}
...
CMatrix<T>::CMatrix(int rows, int columns) :
		_rows(rows), _columns(columns), _data(new T[_rows * _columns]), _ownsData(
				true) {
	if (_data == NULL) {
		printf("error\n");
		exit(1);
...
template<class T>
CVector<T> CMatrix<T>::operator*(const CVector<T> & v) const {
...
#ifdef LAPACK
template<>
inline CVector<double> CMatrix<double>::symmetricEigenvalues() {
	...
	DSYEV(&jobz, &uplo, &_rows, evecsfull._data, &_rows, eigsfull, work, &lwork,
			&info);
...
template<>
inline CVector<double> CMatrix<double>::symmetricEigenvectors(
		CMatrix<double> & evecs, int maxVal, int minVal) {
...
inline void linearRegression(const vector<Neigh*> & inputData,
		const vector<CVector<double>*> & targetData, CMatrix<double> & A,
		CVector<double> & B) {
...
template<class T>
CMatrix<T> CMatrix<T>::outer(const CVector<T> & v1, const CVector<T> & v2) {
