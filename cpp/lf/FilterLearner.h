...
// other routines to do actual work
	...
	Neigh GetNeighborhood(Point2 loc, int currLevel, bool computeWeightVector,
			Pyramid<TFiltImage> * img, Pyramid<TSrcImage> * imgProto = NULL);
	...
	Point2 FindBestMatchLocationAshikhmin(Point2 targetLoc, int level,
			bool twoLevel = false);
	...
	bool FindBestMatchLocationSearchPoints(Point2 targetLoc, int level,
			const vector<Neigh*> & points, Point2 & bestLoc);

	void FindBestMatchLocParent(Point2 targetLoc, Point2 srcLoc, int minLevel,
			int maxLevel, float cumulDist, float &bestDist, Point2 &bestLoc,
			bool &isValid);
	...
	bool GetPredictionsTwoLevel(Point2 targetLoc, int currLevel,
			bool insertRandom, set<Point2> & candidates, bool causal);
	...
	double GetNeighborhoodDist(Point2 targetLoc, Point2 srcLoc, int currLevel,
			bool &isValid, int neighborhoodWidth, double distThresh = -1);
	double GetNeighborhoodDistForLevel(Point2 targetLoc, Point2 srcLoc,
			int currLevel, bool &isValid, bool &hitValidPixel,
			int neighborhoodWidth, bool skipCenterPixel);

	void savePointsToFile(const vector<Neigh*> & points, char *filenameIn =
			"neigh.txt", char *filenameOut = "pixel.txt");
	...
	void optimalBiasGain(const Neigh & target, const Neigh & source,
			const Neigh & weights, double * dist, double * bias, double * gain);
	...
	int NumExamplePairs(void) {
		return m_filteredExamplePyramid.size();
	}
	Pyramid<TSrcImage> & sourceExamplePyramid(int num = 0) {
		return m_sourceExamplePyramid[num];
	}
	const Pyramid<TSrcImage> & sourceExamplePyramid(int num = 0) const {
		return m_sourceExamplePyramid[num];
	}

	Pyramid<TSrcImage> & sourcePyramid() {
		return m_sourcePyramid;
	}
	const Pyramid<TSrcImage> & sourcePyramid() const {
		return m_sourcePyramid;
	}

	Pyramid<TFiltImage> & filteredExamplePyramid(int num = 0) {
		return m_filteredExamplePyramid[num];
	}
	const Pyramid<TFiltImage> & filteredExamplePyramid(int num = 0) const {
		return m_filteredExamplePyramid[num];
	}

	Pyramid<TFiltImage> & filteredPyramid() {
		return m_filteredPyramid;
	}
	const Pyramid<TFiltImage> & filteredPyramid() const {
		return m_filteredPyramid;
	}

	Pyramid<Image<GLubyte> > & validPyramid() {
		return m_destMaskPyramid;
	}
	const Pyramid<Image<GLubyte> > & validPyramid() const {
		return m_destMaskPyramid;
	}

	Pyramid<Image<srcCoord> > & sourcesPyramid() {
		return m_sourcesPyramid;
	}
	Pyramid<Image<srcCoord> > sourcesPyramid() const {
		return m_sourcesPyramid;
	}

	Pyramid<TFiltImage> & modeMaskPyramid() {
		return m_modeMaskPyramid;
	}
	Pyramid<TFiltImage> modeMaskPyramid() const {
		return m_modeMaskPyramid;
	}

	Pyramid<TFiltImage> & exampleModeMaskPyramid() {
		return m_exampleModeMaskPyramid;
	}
	Pyramid<TFiltImage> exampleModeMaskPyramid() const {
		return m_exampleModeMaskPyramid;
	}
	...
	bool useSourceImages() const {
		return m_useSourceImages;
	}
	bool & useSourceImages() {
		return m_useSourceImages;
	}

	bool useSourceImagesAfterFirstPass() const {
		return m_useSourceImagesAfterFirstPass;
	}
	bool & useSourceImagesAfterFirstPass() {
		return m_useSourceImagesAfterFirstPass;
	}
	...
	float levelWeighting() const {
		return m_levelWeighting;
	}
	float & levelWeighting() {
		return m_levelWeighting;
	}

	float finalSourceFac() const {
		return m_finalSourceFac;
	}
	float & finalSourceFac() {
		return m_finalSourceFac;
	}

	double sourceFac() const {
		return m_sourceFac;
	}
	double & sourceFac() {
		return m_sourceFac;
	...
	float samplerEpsilon() const {
		return m_samplerEpsilon;
	}
	float & samplerEpsilon() {
		return m_samplerEpsilon;
	}

	int neighborhoodWidth() const {
		return m_neighborhoodWidth;
	}
	bool useSplineWeights() const {
		return m_useSplineWeights;
	}
	...
	PyramidType sourcePyramidType() const {
		return m_sourcePyramidType;
	}
	PyramidType & sourcePyramidType() {
		return m_sourcePyramidType;
	}

	PyramidType filteredPyramidType() const {
		return m_filteredPyramidType;
	}
	PyramidType & filteredPyramidType() {
		return m_filteredPyramidType;
	}
	...
	bool savePointsToFile() const {
		return m_savePointsToFile;
	}
	bool & savePointsToFile() {
		return m_savePointsToFile;
	}

	int maxTSVQdepth() const {
		return m_maxTSVQdepth;
	}
	int & maxTSVQdepth() {
		return m_maxTSVQdepth;
	}

	float maxTSVQerror() const {
		return m_maxTSVQerror;
	}
	float & maxTSVQerror() {
		return m_maxTSVQerror;
	}

	int TSVQbacktracks() const {
		return m_TSVQbacktracks;
	}
	int & TSVQbacktracks() {
		return m_TSVQbacktracks;
	}
	...
	int MLPnumHidden() const {
		return m_MLPnumHidden();
	}
	int & MLPnumHidden() {
		return m_MLPnumHidden;
	}

	double MLPhiddenDecayWeight() const {
		return m_MLPhiddenDecayWeight;
	}
	double & MLPhiddenDecayWeight() {
		return m_MLPhiddenDecayWeight;
	}

	double MLPoutputDecayWeight() const {
		return m_MLPoutputDecayWeight;
	}
	double & MLPoutputDecayWeight() {
		return m_MLPoutputDecayWeight;
	}

	bool MLPsigmoidal() const {
		return m_MLPsigmoidal;
	}
	bool & MLPsigmoidal() {
		return m_MLPsigmoidal;
	}

	float annEpsilon() const {
		return m_annEpsilon;
	}
	float & annEpsilon() {
		return m_annEpsilon;
	...
	double biasPenalty() const {
		return m_biasPenalty;
	}
	double & biasPenalty() {
		return m_biasPenalty;
	}

	double gainPenalty() const {
		return m_gainPenalty;
	}
	double & gainPenalty() {
		return m_gainPenalty;
	}

	bool createSrcLocHisto(void) const {
		return m_createSrcLocHisto;
	}
	bool & createSrcLocHisto(void) {
		return m_createSrcLocHisto;
	}

	float modeMaskWeight() const {
		return m_modeMaskWeight;
	}
	float & modeMaskWeight() {
		return m_modeMaskWeight;
	}

	float coherenceEps() const {
		return m_coherenceEps;
	}
	float & coherenceEps() {
		return m_coherenceEps;
	}

	float coherencePow() const {
		return m_coherencePow;
	}
	float & coherencePow() {
		return m_coherencePow;
	}
	...
	bool cheesyBoundaries() const {
		return m_cheesyBoundaries;
	}
	bool & cheesyBoundaries() {
		return m_cheesyBoundaries;
	}
	...
	bool ashikhminLastLevel() const {
		return m_ashikhminLastLevel;
	}
	bool & ashikhminLastLevel() {
		return m_ashikhminLastLevel;
	}

	bool onePixelSource() const {
		return m_onePixelSource;
	}
	bool & onePixelSource() {
		return m_onePixelSource;
	}
