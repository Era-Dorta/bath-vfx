	...
	for (int index = 0; index < m_inputParams.size(); index++) {
		if (m_paramDict.find(m_inputParams[index]) != m_paramDict.end()) {
			...
			case FLAG_TYPE:
				...
				if (info.callback) {
					FlagCallbackType *theCallback =
							(FlagCallbackType *) info.callback;
			...
			case INT_TYPE:
				...
				if (info.callback) {
					IntCallbackType *theCallback =
							(IntCallbackType *) info.callback;
			...
			case FLOAT_TYPE:
				...
				if (info.callback) {
					FloatCallbackType *theCallback =
							(FloatCallbackType *) info.callback;
			...
			case DOUBLE_TYPE:
				index++;
				if (index < m_inputParams.size()) {
					*((double *) (info.var)) = atof(
							m_inputParams[index].c_str());
				}

				if (info.callback) {
					DoubleCallbackType *theCallback =
							(DoubleCallbackType *) info.callback;
			...
			case BOOL_TYPE:
				...
				index++;
				if (index < m_inputParams.size()) {
					if (m_inputParams[index] == "false"
							|| m_inputParams[index] == "FALSE") {
						*((bool *) (info.var)) = false;
					} else {
						*((bool *) (info.var)) = true;
					}
				...
				if (info.callback) {
					BoolCallbackType *theCallback =
							(BoolCallbackType *) info.callback;
			...
			case BOOL_INT_TYPE:
				...
				if (info.callback) {
					BoolIntCallbackType *theCallback =
							(BoolIntCallbackType *) info.callback;
					theCallback(info.name.c_str(), (int *) info.var, this);
				}
			...
			case ENUM_TYPE:
				...
				if (info.callback) {
					EnumCallbackType *theCallback =
							(EnumCallbackType *) info.callback;
					theCallback(info.name.c_str(), (int *) info.var, this);
				}
				// ???
				break;
			...
			case STRING_TYPE:
				index++;
				if (index < m_inputParams.size())
					strncpy((char *) (info.var),
							(const char *) m_inputParams[index].c_str(),
							info.maxStringLength);

				if (info.callback) {
					StringCallbackType *theCallback =
							(StringCallbackType *) info.callback;
					theCallback(info.name.c_str(), (char *) info.var,
							info.maxStringLength, this);
				}
...
void ParseArgs::AddIntParam(const char *name, int *var,
...
void ParseArgs::AddFloatParam(const char *name, float *var,
		FloatCallbackType *callback) {
	ParamInfo info;
	info.name = std::string(name);
...
void ParseArgs::AddDoubleParam(const char *name, double *var,
		DoubleCallbackType *callback) {
	ParamInfo info;
	info.name = std::string(name);
...
void ParseArgs::AddBoolParam(const char *name, bool *var,
		BoolCallbackType *callback) {
	ParamInfo info;
	info.name = std::string(name);
	info.type = BOOL_TYPE;
	info.var = var;
...
void ParseArgs::AddBoolParam(const char *name, int *var,
		BoolIntCallbackType *callback) {
	ParamInfo info;
	info.name = std::string(name);
...
void ParseArgs::AddEnumParam(const char *name, EnumValueInfo *enumList,
		int numVals, int *var, EnumCallbackType *callback) {
	ParamInfo info;
	info.name = std::string(name);
	info.type = ENUM_TYPE;
...
void ParseArgs::AddStringParam(const char *name, char *var, int maxLength,
		StringCallbackType *callback) {
	ParamInfo info;
	info.name = std::string(name);
...
void ParseArgs::Dump(FILE *outFile, bool dumpType) {
	std::map<std::string, ParamInfo, std::less<std::string> >::iterator mapIter =
			m_paramDict.begin();
	...
	while (mapIter != m_paramDict.end()) {
		...
		case ENUM_TYPE:
			...
			if ((info.reverseEnumMap).find(*((int *) info.var))
					!= (info.reverseEnumMap).end()) {
				fprintf(outFile, " %s",
						info.reverseEnumMap[*((int *) info.var)].c_str());
			} else
				fprintf(outFile, " ???");
		...
		if (dumpType && info.type == ENUM_TYPE) {
			std::map<std::string, int, std::less<std::string> >::iterator enumIter =
					info.enumMap.begin();
...
void ParseArgs::DumpValuesHTML(ofstream &out) {
	...
	while (mapIter != m_paramDict.end()) {
		...
		case ENUM_TYPE: {
			...
			if ((info.reverseEnumMap).find(value)
					!= (info.reverseEnumMap).end()) {
