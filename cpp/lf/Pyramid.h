...
public:
	Pyramid() :
			_levels(-1), _imgs(NULL), _ownsFine(false), _myType(
					GAUSSIAN_PYRAMID) {
	}
...
//		   PFILTER pf);
	...
	TImage & FineImage(void) {
		assert(_levels > 0);
		return *_imgs[0];
	}
	const TImage & FineImage(void) const {
		assert(_levels > 0);
		return *_imgs[0];
	}
	...
	PyramidType type() const {
		return _myType;
	}
	int displayWidth() const {
		return FineImage().width() + 5;
	}
