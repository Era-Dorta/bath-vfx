...
public:
	...
	const F& x(void) const {
		return v[X];
	}
	F& y(void) {
		return v[Y];
	}
	const F& y(void) const {
		return v[Y];
	}
	F& z(void) {
		return v[Z];
	}
	const F& z(void) const {
		return v[Z];
	}
	...
	CVec3T lerp(const CVec3T& v1, const F& t) const {
		return CVec3T(v[X] + t * (v1.v[X] - v[X]), v[Y] + t * (v1.v[Y] - v[Y]),
				v[Z] + t * (v1.v[Z] - v[Z]));
	}
	...
	//:  length |*this -c|
	F dist(const CVec3T& c) const {
		return (*this - c).l2();
	}

	CVec3T<F> cross(const CVec3T<F>& b) const {
		return CVec3T<F>(y() * b.z() - z() * b.y(), z() * b.x() - x() * b.z(),
				x() * b.y() - y() * b.x());
	...
	template<class G> CVec3T(const CVec3T<G>& u) {
		v[0] = u.x();
		v[1] = u.y();
		v[2] = u.z();
	}
