...
CANNSearchEnvironment::CANNSearchEnvironment(vector<Neigh*> &neighborhoods,
	...
	m_neighborhoodLocations.erase(m_neighborhoodLocations.begin(),
			m_neighborhoodLocations.end());
...
Point2 CANNSearchEnvironment::FindBestMatchLocation(Neigh &neighborhood) {
	assert(m_annTree);
...
CTSVQSearchEnvironment::CTSVQSearchEnvironment(vector<Neigh*> &neighborhoods,
		Sampler<Point2> *sampler, float maxError, int maxDepth) :
		CSearchEnvironment(sampler) {
...
Point2 CTSVQSearchEnvironment::FindBestMatchLocation(Neigh &neighborhood) {
	assert(m_tsvqTree);
