...
Pyramid<TImage>::Pyramid(int levels, int width, int height, int dim) :
...
template<class TImage>
Pyramid<TImage>::Pyramid(const Pyramid & p) {
	_myType = p._myType;
	_levels = p._levels;
	_ownsFine = true;
...
template<class TImage>
void Pyramid<TImage>::makePyramid(int height, TImage * FineImage,
		PyramidType ptype) {
	_myType = ptype;
...
void Pyramid<TImage>::reconstruct() {
	...
	if (_myType == LAPLACIAN_PYRAMID) {
		...
		printf(
				"The reconstruction function only works for Laplacian or Steerable. _myType = %d\n",
				_myType);
	}
...
template<class TImage>
void Pyramid<TImage>::makePyramid(int levels, int imWidth, int imHeight,
		int imDim, PyramidType ptype) {
	assert(imDim > 0);
...
template<class TImage>
void Pyramid<TImage>::drawSourcesColumn(int x, int y, int rmax,
		int gmax) const {
...
template<class TImage>
int Pyramid<TImage>::displayHeight() const {
	int height = 0;
