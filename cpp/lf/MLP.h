...
public:
	MLP(const vector<Neigh*> & inputData,
			const vector<CVector<double>*> & targetData, int num_hidden,
			double hiddenDecayWeight, double outputDecayWeight, bool sigmoidal =
					false);
...
private:
	...
	void computeEnergy(double * weights, double & energy, double * gradient,
			...
			double hiddenDecayWeight, double outputDecayWeight) const;
