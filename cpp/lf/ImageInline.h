enum ImageFileFormat {
	UNKNOWN_FORMAT, BMP_FORMAT, PNM_FORMAT, PNG_FORMAT, RAW_FORMAT
};
// image formats we can read

template<>
inline int Image<GLubyte>::mytype() const {
	return GL_UNSIGNED_BYTE;
}

template<>
inline int Image<GLfloat>::mytype() const {
	return GL_FLOAT;
}

template<>
inline int Image<GLshort>::mytype() const {
	return GL_SHORT;
}

template<>
inline int Image<GLint>::mytype() const {
	return GL_INT;
}

template<>
inline GLubyte Image<GLubyte>::maxVal() const {
	return 255;
}

template<>
inline GLfloat Image<GLfloat>::maxVal() const {
	return 1;
}

template<>
inline GLint Image<GLint>::maxVal() const {
	return 1;
}

inline ImageFileFormat getFileFormat(const char *filename) {
...
Image<T>::Image(int w, int h, int d) :
		_width(w), _height(h), _dim(d), _winx(-1), _winy(-1), _colorspace(
				RGB_SPACE) {
	assert(_dim > 0);
...
Image<T>::Image(const Image & img) :
		_width(img._width), _height(img._height), _dim(img._dim), _winx(
				img._winx), _winy(img._winy), _colorspace(img._colorspace) {
	assert(_dim > 0);
	_pixels = new T[_width * _height * _dim];
	if (_pixels == NULL) {
		printf("Can't allocate image\n");
...
void Image<T>::draw(int x, int y) {
	...
	glDrawPixels(_width, _height, (GLenum) myformat(), (GLenum) mytype(),
			_pixels);
}
...
void Image<T>::read(int x, int y) {
	...
	glReadPixels(x, y, _width, _height, (GLenum) myformat(), (GLenum) mytype(),
			_pixels);
}
...
template<class T>
void Image<T>::sourcesDraw(int x, int y, int sourceWidth, int sourceHeight) {
	_winx = x;
	_winy = y;
	glRasterPos2i(x, y);
...
Image<T> * Image<T>::downsample() {
	...
	double kernel[3][3] = { { .0625, .1250, .0625 }, { .1250, .2500, .1250 }, {
			.0625, .1250, .0625 } };
...
void ColorImage::loadPNM(const char * filename) {
	...
	if (ftype != ASCII_PPM && ftype != RAW_PPM && ftype != ASCII_PGM
			&& ftype != RAW_PGM) {
...
void ColorImage::savePNG(const char * filename) {
	...
	if (_dim != 1 && _dim != 3) {
		printf(
				"saving PNG with image dimension other than 1 or 3 not supported right now\n");
		return;
	}
	...
	for (int x = 0; x < _width; x++)
		for (int y = 0; y < _height; y++)
			for (int d = 0; d < _dim; d++) {
				...
				temp_pixels[_dim * (x + (_height - y - 1) * _width) + d] =
						(GLubyte) val;
			}
...
void ColorImage::loadPNG(const char * filename) {
	...
	for (int x = 0; x < _width; x++)
		for (int y = 0; y < _height; y++)
			for (int d = 0; d < 3; d++)
				Pixel(x, y, d) = temp_pixels[3
						* (x + (_height - y - 1) * _width) + d]
						* (maxVal() / 255.0);
...
ColorImage::Image(const char *filename) :
		_pixels(NULL), _width(-1), _height(-1), _dim(3), _winx(-1), _winy(-1), _colorspace(
				RGB_SPACE) {
...
void Image<T>::insert(Image & im, int x0, int y0, bool rescale) {
	...
	if (dim() == im.dim() && !rescale) {
		for (int y = 0; y < im.height(); y++)
			memcpy(&Pixel(x0, y + y0, 0), &im.Pixel(0, y, 0),
					im.width() * dim() * sizeof(T));
		return;
	}
		...
		for (int y = 0; y < im.height(); y++)
			for (int x = 0; x < im.width(); x++) {
				...
				for (d = 0; d < dim() && d < im.dim(); d++)
					Pixel(x + x0, y + y0, d) = (im.maxVal() + im.Pixel(x, y, d))
							/ 2;
				for (; d < dim(); d++)
					Pixel(x + x0, y + y0, d) = (im.maxVal() + im.Pixel(x, y, 0))
							/ 2;
			}
	} else {
		for (int y = 0; y < im.height(); y++)
...
template<class T>
void Image<T>::meanCovariance(CVector<double> & mean, CMatrix<double> & covar) {
	assert(
			mean.Length() == dim() && covar.columns() == dim()
					&& covar.rows() == dim());
...
void Image<T>::RGBtoYIQ(Image<T> &img) {
	...
	for (int x = 0; x < width(); x++)
		for (int y = 0; y < height(); y++) {
			img.Pixel(x, y, 0) = .299 * Pixel(x, y, 0) + .587 * Pixel(x, y, 1)
					+ .114 * Pixel(x, y, 2);
			img.Pixel(x, y, 1) = .596 * Pixel(x, y, 0) - .274 * Pixel(x, y, 1)
					- .322 * Pixel(x, y, 2);
			img.Pixel(x, y, 2) = .211 * Pixel(x, y, 0) - .523 * Pixel(x, y, 1)
					+ .312 * Pixel(x, y, 2);
		}
}
...
void Image<T>::RGBtoGrayScale(Image<T> &img) {
	...
	if (dim() == 1) {
		...
		for (int x = 0; x < width(); x++)
			for (int y = 0; y < height(); y++) {
				img.Pixel(x, y, 0) = .299 * Pixel(x, y, 0)
						+ .587 * Pixel(x, y, 1) + .114 * Pixel(x, y, 2);
			} // for
	}
}

template<class T>
void Image<T>::applyGainBias(float gain, float bias) {
	assert(dim() == 1);
...
template<class T>
void Image<T>::applyGainBias(const CMatrix<double>& gain,
		const CVector<double> & bias) {
	for (int x = 0; x < _width; x++)
		for (int y = 0; y < _height; y++)
...
template<class T>
void Image<T>::applyGainBias(int x, int y, const CMatrix<double> & gain,
		const CVector<double> & bias) {
	assert(
			bias.Length() == dim() && gain.rows() == dim()
					&& gain.columns() == dim());
...
void Image<T>::ConvertToColorspace(COLORSPACE c) {
	...
	if (_dim != 3) {
		fprintf(stderr,
				"ConvertToColorspace() -- needs image with 3 channels, not %d\n",
				_dim);
		return;
	}
