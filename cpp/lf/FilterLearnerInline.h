...
template<class TSource, class TFilter>
void CFilterLearner<TSource, TFilter>::SetNeighborhood(int nw,
		bool useSplineWeights) {
	...
	if (!m_useSplineWeights)
		...
		for (int width = 1; width <= m_neighborhoodWidth; width += 2) {
			...
			for (int i = 1; i < m_neighborhoodWidth; i++)
				m_kernel->get(width, i) = m_kernel->get(width, i - 1)
						* (width - i) / i;
		}
	}
...
template<class TSource, class TFilter>
void CFilterLearner<TSource, TFilter>::Synthesize(void) {
	...
	if (m_filteredBaseProcedure == COPY_SYNTH && !m_useSourceImages) {
		printf(
				"Error: can't Copy source with no source; reverting to normal\n");
	...
	if ((m_useFilterModeMask || m_useTargetModeMask)
		...
		printf(
				"Warning: can't use mode mask with this search type; reverting\n");
	...
	if (m_filteredBaseProcedure == COPY_SYNTH) {
		printf(
				"Warning: Correct Laplacian pyramid for copying source not implemented yet\n");
		...
		for (m_pass = 0; m_pass < m_numPasses; m_pass++) {
			...
			if (m_keepSearchStats)
				printf(
						...
						m_numTreeHits, (100.0 * m_numPredHits) / m_numSearches,
						m_numPredHits,
						(100.0 * m_numPredOnlyHits) / m_numSearches,
						m_numPredOnlyHits,
						(100.0 * m_numSearchHits) / m_numSearches,
						m_numSearchHits,
...
template<class TSource, class TFilter>
void CFilterLearner<TSource, TFilter>::SynthesizePyramidLevel(int level) {
	...
	if ((m_usePCA || m_searchType == TSVQ_SEARCH
			|| m_searchType == VECTOR_SEARCH || m_searchType == HEURISTIC_SEARCH
			|| m_searchType == MLP_SEARCH || m_searchType == TSVQR_SEARCH
	...
	if (neighborhoods.size() > 0) {
		...
		if ((m_searchType == TSVQ_SEARCH || m_searchType == HEURISTIC_SEARCH)
			...
			CTSVQSearchEnvironment *evt = new CTSVQSearchEnvironment(
					neighborhoods, m_sampler, m_maxTSVQerror, m_maxTSVQdepth);
			...
			if ((m_useFilterModeMask || m_useTargetModeMask)
				...
				CTSVQSearchEnvironment *evt = new CTSVQSearchEnvironment(
						neighborhoodsMM, m_sampler, m_maxTSVQerror,
						m_maxTSVQdepth);
				evt->SetBacktracks(m_TSVQbacktracks);
				evt->SetNumComparisons(m_numComparisons);
		...
		if ((m_searchType == ANN_SEARCH || m_searchType == HEURISTIC_ANN_SEARCH)
				&& !(m_ashikhminLastLevel && level == 0)) {
			CANNSearchEnvironment *evt = new CANNSearchEnvironment(
					neighborhoods, m_sampler);
	...
	bool usingPoints = (m_searchType == VECTOR_SEARCH || m_usePCA
			|| m_searchType == HEURISTIC_SEARCH) && searchEnvironment == NULL
			&& rtree == NULL && neighborhoods.size() > 0;
	...
	if (m_createSrcLocHisto) {
		for (int index = 0; index < m_filteredExamplePyramid.size(); index++) {
			...
			sourceLocHistoImages.push_back(
					new ColorImage(img.width(), img.height(), 1, 0));
		}
	}
	...
	if (m_searchType == ASHIKHMIN_SEARCH && m_pass == 0) {
		...
		for (int iy = 0; iy < result.height(); iy++)
			for (int ix = 0; ix < result.width(); ix++) {
				for (int d = 0; d < result.dim(); d++)
					result.Pixel(ix, iy, d) = ((double) rand()
							/ (double) RAND_MAX) * result.maxVal();
...
//      printf("level = %d, y = %d\n",level,iy);
		for (int ix = startx; ix < result.width() && ix >= 0; ix += inc) //ugh again
			...
			if (m_cheesyBoundaries && level < m_filteredPyramid.NLevels() - 1
				...
				if (ix < radius || ix + radius + numLevels >= result.width()
						|| iy < radius
						|| iy + radius + numLevels >= result.height()) {
					...
					bestLoc[0] = 2
							* m_sourcesPyramid[level + 1].Pixel(srcx, srcy, 0);
					bestLoc[1] = 2
							* m_sourcesPyramid[level + 1].Pixel(srcx, srcy, 1);
			...
			if ((m_useFilterModeMask || m_useTargetModeMask)
				...
				currentNeighborhoods =
						m_useMMnow ? &neighborhoodsMM : &neighborhoods;
			...
			if (!locComputed)
				...
				case HEURISTIC_SEARCH:
					bestLoc = FindBestMatchLocationHeuristic(Point2(ix, iy),
							level, currentSearchEnvironment);
					break;
				...
				case VECTOR_SEARCH:
					if (neighborhoods.size() == 0)
						...
						computed = FindBestMatchLocationSearchPoints(
								Point2(ix, iy), level, *currentNeighborhoods,
								bestLoc);
					break;

				case TSVQR_SEARCH:
					if (rtree != NULL)
						computed = ApplyTSVQR(rtree, Point2(ix, iy), level,
								bestLoc);
					else
						bestLoc = FindBestMatchLocation(Point2(ix, iy), level);
					break;
			...
			if (bestLoc[0] < 0 || bestLoc[1] < 0) {
				...
				if (m_useRandom) {
					...
					int rx = RandInt(
							m_filteredExamplePyramid[imageNum][level].width());
					int ry = RandInt(
							m_filteredExamplePyramid[imageNum][level].height());
					printf(
							"picking random src loc for pixel %d %d at %d; loc = %d %d in image # %d\n",
					...
					printf(
							"copying src loc for pixel %d %d at %d; using %d %d\n",
							ix, iy, level, ix, iy);
			...
			assert(
					m_filteredExamplePyramid[bestLoc.ImageID()][level].inside(
							bestLoc[0], bestLoc[1]));

			result.copyPixel(ix, iy,
					m_filteredExamplePyramid[bestLoc.ImageID()][level],
					bestLoc[0], bestLoc[1]);
			resultMask.Pixel(ix, iy) = resultMask.maxVal();
	...
	if (m_createSrcLocHisto) {
		...
		for (int index = 0; index < sourceLocHistoImages.size(); index++) {
			...
			for (int iy = 0; iy < img->height(); iy++)
				for (int ix = 0; ix < img->width(); ix++) {
					...
					if (val > 0)
						img->Pixel(ix, iy, 0) = (10.0 / 255.0)
								+ (245.0 / 255.0) * (val / maxLocHistoValue);
				}
		...
		sourceLocHistoImages.erase(sourceLocHistoImages.begin(),
				sourceLocHistoImages.end());
	}
	...
	if (m_searchType == ASHIKHMIN_SEARCH
			|| (level == 0 && m_ashikhminLastLevel)) {
...
template<class TSource, class TFilter>
Point2 CFilterLearner<TSource, TFilter>::ApplyPrediction(Point2 targetLoc,
		int currLevel, Point2 bestLoc, float coherenceFac) {
	...
	double bestDist = GetNeighborhoodDist(targetLoc, bestLoc, currLevel,
			isValid, m_neighborhoodWidth) * coherenceFac;

	//assert(isValid);

	for (set<Point2>::iterator it = candidates.begin(); it != candidates.end();
			++it) {
		isValid = false;
...
#ifdef LAPACK
template<class TSource, class TFilter>
CMatrix<double> CFilterLearner<TSource, TFilter>::PCA(vector<Neigh*> & points,
		Neigh & mean, float minRatio)
	...
	CVector<double> eigenvals2 = scatter.symmetricEigenvectors(evT, dim,
			dim - nVecs + 1);
	...
	printf("Kept %d of %d dimensions.  First %d eigenvalues are: ", nVecs, dim,
			nVecs + 1);
	for (i = 0; i < nVecs; i++)
...
template<class TSource, class TFilter>
Neigh CFilterLearner<TSource, TFilter>::GetNeighborhood(Point2 loc,
		int currLevel, bool computeWeightVector, Pyramid<TFiltImage> * img,
		Pyramid<TSrcImage> * imgProto) {
	...
	for (level = currLevel, radius = (m_neighborhoodWidth - 1) / 2;
			level <= maxLevel && radius >= 1 && levelWeight != 0; level++) {
		int width = 2 * radius + 1;
		int levelStart = i;
		...
		if (useSource && !m_onePixelSource) {
			...
			for (int iy = 0; iy < width; ++iy)
				for (int ix = 0; ix < width; ++ix) {
					...
					if (isFirstPixel) {
						for (int d = 0; d < sdim; d++) {
							n.constSrcColor[d] = impcur.Pixel(
									p[0] + ix - radius, p[1] + iy - radius, d);
						}
		...
		} else if (useSource && m_onePixelSource && level == currLevel) {
			n.constSrc = true;
...
template<class TSource, class TFilter>
bool CFilterLearner<TSource, TFilter>::ApplyMLP(MLP * mlp, Point2 targetLoc,
		int currLevel, Point2 & bestLoc) {
#ifndef BFGS
...
template<class TSource, class TFilter>
bool CFilterLearner<TSource, TFilter>::ApplyTSVQR(TSVQR * rtree,
		Point2 targetLoc, int currLevel, Point2 & bestLoc) {
#ifndef LAPACK
	...
	Pyramid<TFiltImage> * img = &m_filteredPyramid;
	Pyramid<TSrcImage> * imgFilt =
			m_useSourceImagesNow ? &m_sourcePyramid : NULL;
...
template<class TSource, class TFilter>
Point2 CFilterLearner<TSource, TFilter>::FindBestMatchLocationHeuristic(
		Point2 targetLoc, int currLevel,
	...
	if (m_filteredExamplePyramid.size() > 1)
		fprintf( stderr,
				"WARNING! currently using only one example pair for heuristic matching\n");
	...
	if (targetLoc[0] > 0) {
		...
		if (srcx < exampleWidth - 1
				&& !m_flags.testAndSet(srcx + 1, srcy, currLevel)) {
	...
	if (targetLoc[1] > 0) {
		...
		if (srcy < exampleHeight - 1
				&& !m_flags.testAndSet(srcx, srcy + 1, currLevel)) {
	...
	if (currLevel < m_sourcesPyramid.NLevels() - 1) {
		int srcx = m_sourcesPyramid[currLevel + 1].Pixel(targetLoc[0] / 2,
				targetLoc[1] / 2, 0);
		int srcy = m_sourcesPyramid[currLevel + 1].Pixel(targetLoc[0] / 2,
				targetLoc[1] / 2, 1);
	...
	while (!ready.empty() || (!usedTree && bestDist > m_annEpsilon)) {
		...
		assert(
				m_filteredExamplePyramid[0][currLevel].inside(ptnew[0],
						ptnew[1]));

		bool isValid = false;
		float dist = GetNeighborhoodDist(targetLoc, ptnew, currLevel, isValid,
	...
	if (m_keepSearchStats) {
		...
		bool treeHit = bool(
				usedTree && finalResult[0] == treeResultLoc[0]
		...
		for (set<Point2>::iterator it2 = predPts.begin(); it2 != predPts.end();
				++it2)
		...
		for (set<Point2>::iterator it3 = searchPts.begin();
				it3 != searchPts.end(); ++it3)
...
template<class TSource, class TFilter>
void CFilterLearner<TSource, TFilter>::optimalBias(const Neigh & target,
		const Neigh & source, const Neigh & weights, double * dist,
		double * bias) {
	...
	if (dist != NULL)
		*dist = (target - weights * targetWmean).dist2(
				source - weights * sourceWmean) + b * b * m_biasPenalty / 2;
}

template<class TSource, class TFilter>
void CFilterLearner<TSource, TFilter>::optimalGain(const Neigh &target,
		const Neigh & source, const Neigh &weights, double * dist,
		double * gain) {
...
template<class TSource, class TFilter>
void CFilterLearner<TSource, TFilter>::optimalBiasGain(const Neigh & target,
		const Neigh & source, const Neigh & weights, double * dist,
...
template<class TSource, class TFilter>
bool CFilterLearner<TSource, TFilter>::FindBestMatchLocationSearchPoints(
		Point2 targetLoc, int currLevel, const vector<Neigh*> & points,
		Point2 &bestLoc) {
	assert(points.size() > 0);
	assert(m_filteredExamplePyramid.size() > 0);
	...
	if (m_useBias || m_useGain)
		weights = new Neigh(
				GetNeighborhood(targetLoc, currLevel, true, img, imgFilt));
	...
	for (int i = 0; i < points.size(); i++) {
		...
		if (!m_useBias && !m_useGain) {
			...
			double d2 = GetNeighborhoodDist(targetLoc, points[i]->loc,
					currLevel, isValid, m_neighborhoodWidth);

			assert(m_usePCA || !isValid || abs(d - d2) < 1e-8);
#endif
		} else if (m_useBias && !m_useGain)
	...
	for (int d = 0; d < (*img)[currLevel].dim(); d++) {
		TFilter src =
				m_filteredExamplePyramid[bestLoc.ImageID()][currLevel].Pixel(
						bestLoc[0], bestLoc[1], d);
...
template<class TSource, class TFilter>
bool CFilterLearner<TSource, TFilter>::GetPredictions(Point2 targetLoc,
		int currLevel, bool insertRandom, set<Point2> & candidates, bool causal,
		int inWidth) {
	...
	for (int iy = 0; iy <= radius || (!causal && iy < width); ++iy)
		for (int ix = 0; ix < width && (ix < radius || iy < radius || !causal);
			...
			if (!sources.inside(srcX, srcY)) {
				...
				if (insertRandom)
					candidates.insert(
							Point2(RandInt(exampleWidth),
									RandInt(exampleHeight),
									RandInt(m_filteredExamplePyramid.size())));
				continue;
			}
...
template<class TSource, class TFilter>
bool CFilterLearner<TSource, TFilter>::GetPredictionsTwoLevel(Point2 targetLoc,
		int currLevel, bool insertRandom, set<Point2> & candidates,
		bool causal) {
	...
	bool prevHitValid = GetPredictions(prevLoc, currLevel + 1, insertRandom,
			prevCandidates, false, m_neighborhoodWidth - 2); //
	...
	if (prevHitValid) {
		for (set<Point2>::iterator it = prevCandidates.begin();
				it != prevCandidates.end(); ++it) {
			Point2 prevLoc = *it;
			candidates.insert(
					Point2(2 * prevLoc.x(), 2 * prevLoc.y(),
							prevLoc.ImageID()));
			candidates.insert(
					Point2(2 * prevLoc.x() + 1, 2 * prevLoc.y(),
							prevLoc.ImageID()));
			candidates.insert(
					Point2(2 * prevLoc.x(), 2 * prevLoc.y() + 1,
							prevLoc.ImageID()));
			candidates.insert(
					Point2(2 * prevLoc.x() + 1, 2 * prevLoc.y() + 1,
							prevLoc.ImageID()));
		}
	}
	...
	bool hitValid = GetPredictions(targetLoc, currLevel, insertRandom,
			candidates, causal);
	return hitValid || prevHitValid;
}

template<class TSource, class TFilter>
Point2 CFilterLearner<TSource, TFilter>::FindBestMatchLocationAshikhmin(
		Point2 targetLoc, int currLevel, bool twoLevel) {
	set<Point2> candidates;

	bool hitValidN;
	if (twoLevel)
		hitValidN = GetPredictionsTwoLevel(targetLoc, currLevel, true,
				candidates, m_pass == 0);
	else
		hitValidN = GetPredictions(targetLoc, currLevel, true, candidates,
				m_pass == 0);
	...
	if (hitValidN)
		for (set<Point2>::iterator it = candidates.begin();
				it != candidates.end(); ++it) {
			//      printf("\ttesting candidate: %d %d\n",(*it)[0], (*it)[1]);

			bool isValid = false;
			float dist2 = GetNeighborhoodDist(targetLoc, *it, currLevel,
					isValid, m_neighborhoodWidth, bestDist);
...
Point2 CFilterLearner<TSource, TFilter>::FindBestMatchLocation(Point2 targetLoc,
	...
	for (int imageID = 0; imageID < m_filteredExamplePyramid.size();
		...
		int maxLevel = min(currLevel + m_numLevels - 1,
				m_filteredExamplePyramid[imageID].NLevels() - 1);
		...
		for (int iy = 0; iy < feImage.height(); iy++)
			for (int ix = 0; ix < feImage.width(); ix++) {
				...
				double dist = GetNeighborhoodDist(targetLoc, srcLoc, currLevel,
						isValid, m_neighborhoodWidth, m_sampler->threshold());
...
template<class T>
inline double PixelDistSqr(const Image<T> & img1, int x1, int y1,
...
template<class TSource, class TFilter>
double CFilterLearner<TSource, TFilter>::GetNeighborhoodDist(Point2 targetLoc,
		Point2 srcLoc, int currLevel, bool &isValid, int neighborhoodWidth,
		double distThresh) {
	...
	for (int level = currLevel, nw = neighborhoodWidth;
			...
			level++, tar[0] /= 2, tar[1] /= 2, src[0] /= 2, src[1] /= 2, nw =
					(nw >> 1) + 1, levelWeight *= m_levelWeighting
					* m_levelWeighting) {

		double d2 = GetNeighborhoodDistForLevel(tar, src, level, isValid,
				hitValidPixel, nw, bool(level == currLevel));
...
template<class TSource, class TFilter>
double CFilterLearner<TSource, TFilter>::GetNeighborhoodDistForLevel(
		Point2 targetLoc, Point2 srcLoc, int currLevel, bool &isValid,
		bool & hitValidPixel, int neighborhoodWidth, bool skipCenterPixel) {
	assert(m_filteredExamplePyramid.size() > 0);
	int imageID = srcLoc.ImageID();
	assert(imageID >= 0);
	...
	for (int iiy = 0; iiy < neighborhoodWidth; iiy++) {
		...
		for (int iix = 0; iix < neighborhoodWidth; iix++) {
			...
			if (std::isnan(weight))
				printf(
						"weight = %f, m_sourceFac = %f,iix = %d,iiy = %d,k[iix]=%f,k[iiy]=%f\n",
						weight, m_sourceFacNow, iix, iiy,
						m_kernel->get(neighborhoodWidth, iix),
						m_kernel->get(neighborhoodWidth, iiy));
			...
			if (targetMask.Pixel(targetX, targetY)
					&& !(2 * iix + 1 == neighborhoodWidth
							&& 2 * iiy + 1 == neighborhoodWidth
							&& skipCenterPixel)) {
			...
			if (m_useSourceImagesNow
				...
				if (protoPixelValid) {
					double pdist = PixelDistSqr<TSource>(*srcProtoImage, srcX,
							srcY, *targetProtoImage, targetX, targetY);
...
template<class TSource, class TFilter>
Point2 CFilterLearner<TSource, TFilter>::FindBestMatchLocation2(
		Point2 targetLoc, int level) {
	// search over src image, looking for similar neighborhoods
	int maxLevel = min(level + m_numLevels - 1,
	...
	for (int iy = 0; iy < height; iy++)
		for (int ix = 0; ix < width; ix++) {
			...
			bool isValid = false;
			FindBestMatchLocParent(targetLoc, Point2(ix, iy), level, maxLevel,
					-1, bestDist, bestLoc, isValid);
		}
...
template<class TSource, class TFilter>
void CFilterLearner<TSource, TFilter>::FindBestMatchLocParent(Point2 targetLoc,
		Point2 srcLoc, int minLevel, int maxLevel, float cumulDist,
		float &bestDist, Point2 &bestLoc, bool &isValid) {
	assert(minLevel <= maxLevel);

	float divisor = pow2(maxLevel - minLevel);
	Point2 newTargetLoc = Point2(targetLoc[0] / divisor,
			targetLoc[1] / divisor);

	bool thisValid = false, hitValid;
	float dist = GetNeighborhoodDistForLevel(newTargetLoc, srcLoc, maxLevel,
			thisValid, hitValid, 5, false);
	...
	if (minLevel == maxLevel) // leaf node -- we've got the final score
		...
		FindBestMatchLocParent(targetLoc,
				Point2(2 * srcLoc[0], 2 * srcLoc[1] + 1), minLevel,
				maxLevel - 1, dist, bestDist, bestLoc, isValid);
		FindBestMatchLocParent(targetLoc,
				Point2(2 * srcLoc[0] + 1, 2 * srcLoc[1]), minLevel,
				maxLevel - 1, dist, bestDist, bestLoc, isValid);
		FindBestMatchLocParent(targetLoc,
				Point2(2 * srcLoc[0] + 1, 2 * srcLoc[1] + 1), minLevel,
				maxLevel - 1, dist, bestDist, bestLoc, isValid);
	}
}
...
template<class TSource, class TFilter>
void CFilterLearner<TSource, TFilter>::savePointsToFile(
		const vector<Neigh*> & points, char *filename1, char *filename2) {
...
template<class TSource, class TFilter>
void CFilterLearner<TSource, TFilter>::SetImages(
		vector<TFiltImage *> filteredExampleImages, int levels) {
	...
	m_destImage = new TFiltImage(width, height, filteredExampleImages[0]->dim(),
			0);
	if (m_destImage == NULL) {
...
template<class TSource, class TFilter>
void CFilterLearner<TSource, TFilter>::SetImages(
		vector<TSrcImage *> sourceExampleImages,
		vector<TFiltImage *> filteredExampleImages, TSrcImage * sourceImage,
		int levels, TFiltImage * modeMask, TFiltImage * targetModeMask) {
	int index;
	...
	for (index = 0; index < numExamples; index++) {
		assert(
				sourceExampleImages[index]->width()
						== filteredExampleImages[index]->width()
						&& sourceExampleImages[index]->height()
								== filteredExampleImages[index]->height());

		assert(
				filteredExampleImages[index]->width() >= 0
	...
	m_destMaskPyramid.makePyramid(m_filteredPyramid.NLevels(),
			...
			m_filteredPyramid.FineImage().height(), 1, GAUSSIAN_PYRAMID);
	m_destMaskPyramid.clear();
#ifdef STEERDEBUG
	//debug
	printf("Calling makePyramid for m_sourcesPyramid\n");
	...
	m_sourcesPyramid.makePyramid(m_filteredPyramid.NLevels(),
			m_filteredPyramid.FineImage().width(),
			m_filteredPyramid.FineImage().height(), 3, SOURCES_PYRAMID);
	...
	if (m_useFilterModeMask) {
		...
		if (!m_useTargetModeMask) {
			m_modeMaskPyramid.makePyramid(m_filteredPyramid.NLevels(),
					m_filteredPyramid.FineImage().width(),
					m_filteredPyramid.FineImage().height(), 1,
					GAUSSIAN_PYRAMID);
...
template<class TSource, class TFilter>
void CFilterLearner<TSource, TFilter>::AddExamplePair(
		TSrcImage * sourceExampleImage, TFiltImage * filteredExampleImage) {
	int levels = m_filteredPyramid.NLevels();
	assert(levels > 0);

	assert(
			sourceExampleImage->width() == filteredExampleImage->width()
					&& sourceExampleImage->height()
							== filteredExampleImage->height());

	assert(
			filteredExampleImage->width() >= 0
	...
	if (sourceExampleImage->width() != filteredExampleImage->width()
		...
		fprintf( stderr,
				"warning: Adding example pair with different sizes -- ignored\n");
		return;
	}
