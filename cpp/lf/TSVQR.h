...
public:
	TSVQR(const vector<Neigh*> & inputData,
			const vector<CVector<double>*> & targetData, int maxDepth = 20,
			Neigh *cent1 = NULL);

	CVector<double> apply(const CVector<double> & pt,
			unsigned int & num_comparisons, int numBacktracks = 8) const;
	Neigh findNearest(const CVector<double> & pt,
			unsigned int & num_comparisons, int numBacktracks = 8) const;
	const TSVQR * findNearestHelper(const CVector<double> & pt,
			unsigned int & num_comparisons, int & numBacktracks) const;
