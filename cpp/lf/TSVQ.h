...
class TSVQ {
public:
	TSVQ(const vector<PointN*> & points, float maxError = 0, int maxDepth = 20,
			PointN *cent1 = NULL);
	const PointN & findNearest(const PointN & pt,
			unsigned int & numComparisons);
	const PointN &findNearest(const PointN & pt, unsigned int & numComparsions,
			int numBacktracks = 8);
	const PointN &findNearestHelper(const PointN & pt,
			unsigned int & numComparisons, int &numBacktracks);
	const void sample(const PointN &pt, Sampler<Point2> & samp,
			unsigned int & numComparisons, int numBacktracks = 8);
